...
sealed class Debility(override val name: String,
  ...
  data class MaliciousMutualDeception(override val planet: Planet, val sign1: ZodiacSign, val dig1: Dignity,
                                      val p2: Point, val sign2: ZodiacSign, val dig2: Dignity) : Debility(MaliciousMutualDeception::class.java.simpleName) {
    private val mutualData = MutualDataWithSign(planet, sign1, dig1, p2, sign2, dig2)
    override fun equals(other: Any?): Boolean {
      if (this === other) return true
      if (other !is MaliciousMutualDeception) return false

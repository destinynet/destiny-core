  ...
  protected Optional<Pair<String, Object[]>> getResult(Planet planet, @NotNull HoroscopeContext horoscopeContext)
  {
    ...
    if ( Horoscope.getAngle(planetDegree , southDeg) <= 1)
    {
      if (nodeType == NodeType.TRUE)
      {
        ...
        return Optional.of(ImmutablePair.of("comment", new Object[]{planet, LunarNode.SOUTH_TRUE, Aspect.CONJUNCTION}));
      }
      else
      {
        //addComment(Locale.TAIWAN , planet + " 與 " + LunarNode.SOUTH_MEAN + " 形成 " + Aspect.CONJUNCTION);
        return Optional.of(ImmutablePair.of("comment" , new Object[]{planet, LunarNode.SOUTH_MEAN , Aspect.CONJUNCTION}));
      }
    }

...
public final class Partile_Conj_Spica extends Rule {
  ...
  protected Optional<Tuple2<String, Object[]>> getResult(Planet planet, @NotNull Horoscope h) {
    ...
    Optional<Double> planetDegree = h.getPositionOptional(planet).map(Position::getLng);
    Optional<Double> spicaDeg = h.getPositionOptional(FixedStar.SPICA).map(Position::getLng);

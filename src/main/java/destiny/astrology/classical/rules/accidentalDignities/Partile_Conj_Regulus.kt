...
class Partile_Conj_Regulus : Rule() {
  ...
  override fun getResult(planet: Planet, h: Horoscope): Optional<Tuple2<String, Array<Any>>> {
    ...
    return h.getPositionOpt(planet).map<Double> { it.lng }.flatMap { planetDegree ->
      h.getPositionOpt(FixedStar.REGULUS).map<Double> { it.lng }.flatMap { regulusDeg ->
        if (AspectEffectiveModern.isEffective(planetDegree!!, regulusDeg!!, Aspect.CONJUNCTION, 1.0)) {
          logger.debug("{} 與 {} 形成 {}", planet, FixedStar.REGULUS, Aspect.CONJUNCTION)
          Optional.of<Tuple2<String, Array<Any>>>(Tuple.tuple<String, Array<Any>>("comment", arrayOf<Any>(planet, FixedStar.REGULUS, Aspect.CONJUNCTION)))
        }
  ...
  override fun getResult2(planet: Planet, h: Horoscope): Pair<String, Array<Any>>? {
    ...
    return if (planetDeg != null && regulusDeg != null && AspectEffectiveModern.isEffective(planetDeg , regulusDeg , Aspect.CONJUNCTION , 1.0)) {
      logger.debug("{} 與 {} 形成 {}", planet, FixedStar.REGULUS, Aspect.CONJUNCTION)
      "comment" to arrayOf(planet , FixedStar.REGULUS , Aspect.CONJUNCTION)
    } else
      null
  }
}

...
interface IEclipse {
  ...
  fun getRangeSolarEclipses(fromGmt: Double, toGmt: Double, types: Collection<SolarType> = listOf(*SolarType.values())): List<AbstractSolarEclipse> {
    if (fromGmt >= toGmt)
      throw RuntimeException("fromGmt : $fromGmt must less than toGmt : $toGmt")

    val list = ArrayList<AbstractSolarEclipse>()
    var gmt = fromGmt
  ...
  /** 全球，某時間範圍內的月食記錄  */
  fun getRangeLunarEclipses(fromGmt: Double, toGmt: Double, types: Collection<AbstractLunarEclipse.LunarType>?): List<AbstractLunarEclipse> {
    if (fromGmt >= toGmt)
      throw RuntimeException("fromGmt : $fromGmt must less than toGmt : $toGmt")

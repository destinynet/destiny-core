 ...
 */
interface IStarPosition<out T : Position> {
  ...
  fun getPosition(star: Star, gmtJulDay: Double, geoLng: Double, geoLat: Double, geoAlt: Double, centric: Centric, coordinate: Coordinate): T {
    return getPosition(star, gmtJulDay, geoLng, geoLat, geoAlt, centric, coordinate, 0.0, 1013.25)
  }

  fun getPosition(star: Star, gmtJulDay: Double, loc: Location, centric: Centric, coordinate: Coordinate): T {
    return getPosition(star, gmtJulDay, loc.longitude, loc.latitude, loc.altitudeMeter, centric, coordinate, 0.0, 1013.25)
  }

  /**
   * @param gmt GMT 的 Gregorian 時刻
   */
  fun getPosition(star: Star, gmt: ChronoLocalDateTime<*>, centric: Centric, coordinate: Coordinate): Position {
   ...
   */
  fun getPosition(star: Star, gmt: ChronoLocalDateTime<*>, centric: Centric, coordinate: Coordinate, loc: Location, temperature: Double, pressure: Double): Position {
    val gmtJulDay = TimeTools.getGmtJulDay(gmt)
    return getPosition(star, gmtJulDay, loc.longitude, loc.latitude, loc.altitudeMeter, centric, coordinate, temperature, pressure)
  }


  /** 取得星體的位置 , 包含當地時間 (LMT) 以及座標 , 包含 溫度、壓力 */
  fun getPosition(star: Star, lmt: ChronoLocalDateTime<*>, location: Location, centric: Centric, coordinate: Coordinate, temperature: Double = 0.0, pressure: Double = 1013.25): Position {

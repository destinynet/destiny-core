...
public interface IStarPosition<T extends Position> {
  ...
  default T getPosition(Star star, double gmtJulDay, double geoLng , double geoLat , double geoAlt , Centric centric, Coordinate coordinate, double temperature, double pressure) {
    return getPosition(star , gmtJulDay , centric , coordinate);
  }

  default T getPosition(Star star, double gmtJulDay, double geoLng , double geoLat , double geoAlt , Centric centric, Coordinate coordinate) {
    return getPosition(star , gmtJulDay , geoLng , geoLat , geoAlt , centric , coordinate , 0 , 1013.25);
  }

  default T getPosition(Star star, double gmtJulDay, Location loc, Centric centric, Coordinate coordinate) {
    return getPosition(star , gmtJulDay , loc.getLongitude() , loc.getLatitude() , loc.getAltitudeMeter() , centric , coordinate , 0 , 1013.25);
  }
   ...
   */
  default Position getPosition(Star star, ChronoLocalDateTime gmt , Centric centric , Coordinate coordinate , Location loc , double temperature , double pressure){
    double gmtJulDay = TimeTools.getGmtJulDay(gmt);
    return getPosition(star , gmtJulDay , loc.getLongitude() , loc.getLatitude() , loc.getAltitudeMeter() , centric, coordinate, temperature , pressure);
  }

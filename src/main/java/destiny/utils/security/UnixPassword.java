/**
 * @author smallufo 
 * Created on 2008/3/3 at 上午 10:53:45
 */ 
/**
...
/* *** Methods start here *** */
private static void transpose ( @NotNull byte[] data, byte[] t, int n )
{
    byte[] x = new byte[64] ;
...
private static void rotate ( @NotNull byte[] key )
{
    byte[] x = new byte[64] ;
...
private static void f ( int i, @NotNull byte[] key, @NotNull byte[] a, @NotNull byte[] x )
{
    byte[]  e = new byte[64] ;
...
private static void definekey ( @NotNull byte[] k )
{
 ...
 */
@NotNull
public static String crypt ( @NotNull String strpw, @NotNull String strsalt )
{

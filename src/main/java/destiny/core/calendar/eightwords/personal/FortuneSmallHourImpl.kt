...
class FortuneSmallHourImpl(
  ...
  private fun implByRangesMap(gmtJulDay: Double , eightWords:EightWords , gender: Gender , location: ILocation , count: Int , forward:Boolean) : List<FortuneData>  {
    var sb = eightWords.hour
    return intAgeImpl.getRangesMap(gender , gmtJulDay , location , 1 , count).asSequence().map { (age , pair) ->
   ...
   *  */
  private fun implBySequence(gmtJulDay: Double , eightWords:EightWords , gender: Gender , location: ILocation , count: Int , forward:Boolean) : List<FortuneData> {
    var age = 0
    var sb = eightWords.hour

...
//    }
    ...
    if (palm instanceof PalmWithMeta) {
      ...
      renderMeta(metaFrame , (PalmWithMeta) palm, metaW , metaH);
    }
  ...
  private static void renderMeta(Graphics2D g , PalmWithMeta palm , int width , int height) {
    ...
    g.drawString("GMT:" + gmt , 1 , fontH*3);

    g.drawString("農曆：" + palm.getChineseDate() + palm.getHour() + "時" , 1 , fontH * 4);
    ...
    g.drawString("地點：" + locString , 1 , fontH*5);

    g.drawString("順逆：" + palm.getPositiveImpl().getTitle(Locale.TAIWAN) , 1 , fontH * 6);
    g.drawString("時辰劃分：" + palm.getHourImpl().getTitle(Locale.TAIWAN) , 1 , fontH * 7);
    g.drawString("子正是：" + palm.getMidnightImpl().getTitle(Locale.TAIWAN) , 1 , fontH * 8);
    g.drawString("換日：" + (palm.isChangeDayAfterZi() ? "子初換日" : "子正換日") , 1 , fontH * 9);
  }

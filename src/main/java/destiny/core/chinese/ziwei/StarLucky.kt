 ...
 * 八吉星
 */
class StarLucky(nameKey: String, type: ZStar.Type) : ZStar(nameKey, ZStar::class.java.name, nameKey + "_ABBR", type) {
  companion object {

    val 文昌 = StarLucky("文昌", 時) // 甲
    val 文曲 = StarLucky("文曲", 時) // 甲
    val 左輔 = StarLucky("左輔", 月) // 甲
    val 右弼 = StarLucky("右弼", 月) // 甲
    val 天魁 = StarLucky("天魁", 年干) // 甲 , 丙火 , 天乙貴人 , 陽貴
    val 天鉞 = StarLucky("天鉞", 年干) // 甲 , 丁火 , 玉堂貴人 , 陰貴
    val 祿存 = StarLucky("祿存", 年干) // 甲
    val 年馬 = StarLucky("年馬", 年支) // 乙級星 (其實就是天馬)
    val 月馬 = StarLucky("月馬", 月)   // 乙級星 (其實就是天馬)

...
object NineStarTools {
   ...
   * 日紫白入中
   */
  fun getCenterStar(solarTerms: SolarTerms, day: StemBranch): NineStar {
   ...
   * 時紫白入中
   */
  fun getCenterStar(zodiacDegree: Double, day: Branch, hour: Branch): NineStar {
  ...
  fun getHourStar(zodiacDegree: Double, day: Branch, hour: Branch, symbol: Symbol): NineStar {
    val center = getCenterStar(zodiacDegree, day, hour)
    val steps: Int = FlyingStar.symbolPeriods.indexOf(symbol)
    return if (zodiacDegree >= 270 || zodiacDegree < 90) {
      // 冬至後 to 夏至前
      NineStar.of(center.period + steps)

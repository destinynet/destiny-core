...
class FortuneLargeSpanImpl(private val eightWordsFeature: EightWordsFeature,
  ...
  private fun getTargetMajorSolarTermsSeconds(gmtJulDay: GmtJulDay, gender: Gender, index: Int): Double {
    ...
    //現在的 節氣
    var currentSolarTerms = solarTermsImpl.getSolarTerms(gmtJulDay)
      ...
      if (!reverse) {
        ...
        while (i <= index) {
          ...
          if (hashMap[i] == null) {
            ...
            //沒有計算過
            targetGmtJulDay = starTransitImpl.getNextTransitGmt(Planet.SUN, stepMajorSolarTerms.zodiacDegree.toZodiacDegree(), stepGmtJulDay, true, Coordinate.ECLIPTIC, StarTypeOptions.MEAN)
            //以隔天計算現在節氣
          ...
          currentSolarTerms = solarTermsImpl.getSolarTerms(stepGmtJulDay)
        ...
        while (i >= index) {
          ...
          if (hashMap[i] == null) {
            ...
            targetGmtJulDay = starTransitImpl.getNextTransitGmt(Planet.SUN, stepMajorSolarTerms.zodiacDegree.toZodiacDegree(), stepGmtJulDay, false, Coordinate.ECLIPTIC, StarTypeOptions.MEAN)
            //以前一天計算現在節氣
            stepGmtJulDay = targetGmtJulDay - 1
            hashMap[i] = targetGmtJulDay
            cache.put(Pair(gmtJulDay, gender), hashMap)

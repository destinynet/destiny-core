          ...
          grandTrine.points.map { tail ->
            ...
            oppoSet.map { (head, _, oppoScore) ->
              ...
              grandTrine.points.minus(tail).map { wingPoint ->
                ...
                ?.let { map: Map<AstroPoint, Double> ->
                  val wings = map.keys
                  /** 分數 : [AstroPattern.GrandTrine] + 對沖分數 +  head與兩個翅膀 [Aspect.SEXTILE] 的分數 , 四者平均 */
                  val score = grandTrine.score?.value?.let { setOf(it) }?.plus(oppoScore)?.plus(map.values)?.average()
                  AstroPattern.Kite(head.signHouse(posMap, cuspDegreeMap), wings, tail.signHouse(posMap, cuspDegreeMap), score?.toScore())
                }
            }
          }
        }.toSet()
        ...
        .flatMap { pattern ->
          aspectsCalculator.getPointAspectAndScore(pattern.pointer.point, posMap, posMap.keys, setOf(OPPOSITION))
            ...
            .map { (oppoPoint, oppoScore, _) ->
              ...
              // 分數 : 上帝之指分數 + 對沖分數 平均
              val score = pattern.score?.value?.let { patternScore -> setOf(oppoScore).plus(patternScore).average() }
              AstroPattern.Boomerang(pattern, oppoPoint.signHouse(posMap, cuspDegreeMap), score?.toScore())
            }
        }.toSet()
    }
  } // 回力鏢 : YOD + 對沖點
              ...
              aspectsCalculator.getPointAspect(tSquared.squared.point, posMap, aspects = setOf(OPPOSITION)).keys.mapNotNull { oppo: AstroPoint ->
                ...
                aspectsCalculator.getPointAspectAndScore(oppo, posMap, tSquared.oppoPoints, setOf(SQUARE))
                  ...
                  ?.let { twoSquared: Set<Triple<AstroPoint, Aspect, Double>> ->
                    ...
                    // 三個值 , 平均
                    val score = tSquaredScore?.value?.let { twinSquaredScore.plus(it).average() }
                    ...
                    AstroPattern.GrandCross(union4, quality, score?.toScore())
                  }
              }.asSequence()
            }.toSet()
        ...
        ?.let { patterns ->
          ...
          Sets.combinations(patterns.toSet(), 2).asSequence().map { twoWedges ->
            ...
            .map { (twoWedges , oppoScore) ->
              ...
              val score: Double? = twoWedges.takeIf { pattern -> pattern.all { it.score != null } }?.map { it.score!!.value }?.plus(oppoScore)?.average()
              AstroPattern.MysticRectangle(unionPoints, score?.toScore())
            }.toSet()
        } ?: emptySet()
    }
  } // 神秘長方形
    ...
    override fun getPatterns(posMap: Map<AstroPoint, IPos>, cuspDegreeMap: Map<Int, ZodiacDegree>): Set<AstroPattern> {
      return posMap.entries.groupBy { (_, pos) -> pos.sign }
        ...
        .map { (sign, list: List<Map.Entry<AstroPoint, IPos>>) ->
          ...
          AstroPattern.StelliumSign(points, sign, score.toScore())
        }.toSet()
    }
  }
    ...
    override fun getPatterns(posMap: Map<AstroPoint, IPos>, cuspDegreeMap: Map<Int, ZodiacDegree>): Set<AstroPattern> {
      return posMap.map { (point, pos) -> point to IHoroscopeModel.getHouse(pos.lngDeg, cuspDegreeMap) }
        ...
        .map { (house, list: List<Pair<AstroPoint, Int>>) ->
          ...
          AstroPattern.StelliumHouse(points, house, score.toScore())
        }.toSet()
    }
  }
            ...
            Sets.combinations(clusters.toSet(), 2).asSequence().map { twoClusters: Set<Cluster<PointCluster>> ->
              ...
              .mapNotNull { (group1, group2, _) ->
                ...
                if (score.isNaN()) {
                  ...
                  AstroPattern.Confrontation(setOf(group1.toSet(), group2.toSet()), score.toScore())
                }
              }
          }?.toSet() ?: emptySet()
      }

...
class AzimuthDegreeTaiwanDecorator : Decorator<Double> {
  ...
  override fun getOutputString(value: Double): String {
    ...
    return crossMap.asSequence().firstOrNull { (range, _) ->
      ...
      if (value <= 45)
        sb.append("北偏東").append(value.truncateToString(1))
      else if (value > 45 && value < 90)
        sb.append("東偏北").append((90 - value).truncateToString(1))
      else if (value > 90 && value <= 135)
        sb.append("東偏南").append((value - 90).truncateToString(1))
      else if (value > 135 && value < 180)
        sb.append("南偏東").append((180 - value).truncateToString(1))
      else if (value > 180 && value <= 225)
        sb.append("南偏西").append((value - 180).truncateToString(1))
      else if (value > 225 && value < 270)
        sb.append("西偏南").append((270 - value).truncateToString(1))
      else if (value > 270 && value <= 315)
        sb.append("西偏北").append((value - 270).truncateToString(1))
      else
        sb.append("北偏西").append((360 - value).truncateToString(1))
      sb.append("度")
      sb.toString()
    }
  }

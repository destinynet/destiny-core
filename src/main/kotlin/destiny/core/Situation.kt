 ...
 *
 * ## ⛔ `.title` 的語域對齊**輸入表單**，不得進入預測輸出的散文
 *
 * 本 enum 有兩條**語域不同**的通道，混用會出事：
 *
 * | 通道 | 誰讀 | 語域 |
 * |---|---|---|
 * | enum 名稱（`name`） | **只有 LLM** —— 抽取端送的是裸名稱，KDoc 執行期不存在 | 直白、可含臨床詞 |
 * | `.title`（i18n bundle） | **使用者，在輸入表單上** | 溫和，但**可以直白到讓人認出自己那格** |
 *
 * `.title` 之所以能比預測輸出直白，是因為**使用者是在告訴我們發生過什麼，不是我們在預言他**
 * （[DEATH] 以「身故」出現在表單、[MENTAL_HEALTH_EPISODE] 的例示含「自傷」，都是這個理由）。
 *
 * ⚠️ **那個理由一旦離開表單就不成立。** 把 `situation.getTitle(lang)` 拿去組預測提示詞、
 * 或用它渲染 `ForecastWindow` 的 `headline` / `watchFor`，等於對使用者說
 * 「你可能會自傷」—— 而這個失效是**靜默的**：提示詞照樣通、報告照樣生成、測試照樣綠。
 *
 * ⇒ 判讀側要顯示型別時，自己準備一組預測語域的字串，不要借用本 enum 的 `.title`。
 * （同 [category] 與 [roles] 的禁令，只是理由不同：那兩個是「先驗不得餵給判讀端」，
 * 本條是「語域不得跨消費者」。）
 */
@Serializable
enum class Situation(
   ...
   *
   * ⚠️ **判準是「信任關係 ＋ 在暗處」，不是「有沒有欺騙」。**
   * 詐騙集團騙一個陌生人不屬此列（沒有信任關係），受方那側走 [MAJOR_FINANCIAL_LOSS]。
   * 這條界線可觀察：**加害者是否在受方的信任範圍內**。
   *
   * ## ⭐ 日期取「**發現的那一天**」，不是事情發生的那一天
   *
   * 這兩者可以差好幾年，而**排盤吃的就是這個日期** —— 選錯就是拿一張無關的盤去比對。
   *
   * 取發現日的理由與 [MAJOR_ILLNESS] 同構：那一格的 KDoc 明寫它是
   * 「**知道了** —— 診斷落地」，是**資訊事件**而非病程起點；病程另有
   * [LONG_TERM_MEDICAL_TREATMENT] 承載。背叛同理 —— 對受方而言，
   * 那一天真正發生的事是**信任崩塌**，而信任是在他知道的那一刻才崩的。
   *
   * ⚠️ 這條只約束 [EventRole.RECIPIENT] 那一側。
   * [EventRole.INITIATOR] 沒有歧義：他動手的那一天。
   * ⇒ 同一件事若兩造都在同一條時間軸上（罕見，但合夥決裂的案例會出現），
   * 兩筆的日期本來就該不同，那不是矛盾。
   */

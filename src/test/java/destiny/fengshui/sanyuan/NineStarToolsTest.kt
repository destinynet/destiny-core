...
import destiny.core.calendar.SolarTerms.立冬
...
class NineStarToolsTest {
  ...
  fun testGetDayStar_夏至到冬至() {
    NineStarTools.getCenterStar(SolarTerms.小雪, StemBranch.乙丑).also {
      assertSame(廉貞, it)
    }
    ...
    assertSame(破軍, NineStarTools.getCenterStar(立冬, 己亥))
    assertSame(巨門, NineStarTools.getDayStar(立冬, 己亥, 坎))
    assertSame(文曲, NineStarTools.getDayStar(立冬, 己亥, 艮))
    assertSame(右弼, NineStarTools.getDayStar(立冬, 己亥, 震))
    assertSame(左輔, NineStarTools.getDayStar(立冬, 己亥, 巽))
    assertSame(祿存, NineStarTools.getDayStar(立冬, 己亥, 離))
    assertSame(貪狼, NineStarTools.getDayStar(立冬, 己亥, 坤))
    assertSame(廉貞, NineStarTools.getDayStar(立冬, 己亥, 兌))
    assertSame(武曲, NineStarTools.getDayStar(立冬, 己亥, 乾))
  }

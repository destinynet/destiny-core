  ...
  inner class ResponseFormatTest {
    ...
    fun text() {
      val chatModel = OpenAi.ChatModel(
        ...
        null, "gpt-4o", 0.9, null, null, null, emptyList(), null
      )
    ...
    fun jsonSchema() {
      val chatModel = OpenAi.ChatModel(
        listOf(OpenAi.Message("user", "test message", null, emptyList())),
        null, "gpt-4o", 0.9, null, null, null, emptyList(), SynastryReply::class.toJsonSchema("SynastryReply", "reply of a synastry chart")
      )
      json.encodeToString(chatModel).also { raw ->
        logger.info { raw }

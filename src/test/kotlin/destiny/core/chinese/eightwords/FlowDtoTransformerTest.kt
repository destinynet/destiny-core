  ...
  inner class FlowLargeTest {
    ...
    inner class 大運天干合住本命天干 {
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.StemCombinedDto(
            "本命時干(己) 被大運(甲)合住，甲己合化土",
            NatalStems(setOf(HOUR), 己),
            EwEvent.EwFlow.FlowStems(setOf(FlowScale.LARGE), 甲),
            FiveElement.土
          )
        )
    ...
    inner class 大運與本命地支相合 {
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            "大運地支(戌) 合住 本命時支(卯)",
            NatalBranches(setOf(HOUR), 卯),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 戌),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            ...
            NatalBranches(setOf(MONTH, DAY, HOUR), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 午),
          )
        )
        assertEquals(expected, dtos)
      }
    ...
    inner class 本命已經三合取二_再拱大運 {
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.TrilogyToFlowDto(
            "大運(辰)與本命年柱(子)、時柱(申)三合水局",
            setOf(
              NatalBranches(setOf(YEAR), 子),
              NatalBranches(setOf(HOUR), 申)
            ),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 辰)
          )
        )
        assertEquals(expected, dtos)
      }
    ...
    inner class 大運地支正沖本命 {
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchOppositionDto(
            "大運地支(酉) 正沖 本命時支(卯)",
            NatalBranches(setOf(HOUR), 卯),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 酉)
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchOppositionDto(
            ...
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 丑)
          )
        )
        assertEquals(expected, dtos)
      }
  ...
  inner class FlowLargeYearTest {
    ...
    fun 大運流年同時影響本命天干() {
      ...
      val expected = setOf(
        EwEvent.EwFlow.AffectingDto(
          ...
          NatalStems(setOf(YEAR, DAY), 丙),
          Reacting.PRODUCED,
          setOf(FlowScale.LARGE, FlowScale.YEAR)
        ),
        EwEvent.EwFlow.AffectingDto(
          "本命月干(乙) 與 大運、流年五行相同",
          NatalStems(setOf(MONTH), 乙),
          Reacting.SAME,
          setOf(FlowScale.LARGE, FlowScale.YEAR)
        ),
        EwEvent.EwFlow.AffectingDto(
          "本命時干(壬) 生/洩 出大運、流年",
          NatalStems(setOf(HOUR), 壬),
          Reacting.PRODUCING,
          setOf(FlowScale.LARGE, FlowScale.YEAR)
        )
      )
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.StemCombinedDto(
            ...
            NatalStems(setOf(HOUR), 己),
            EwEvent.EwFlow.FlowStems(setOf(FlowScale.LARGE, FlowScale.YEAR), 甲),
            FiveElement.土
          )
        )
        assertEquals(expected, dtos)
    ...
    inner class 大運流年合住本命地支 {
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            "流年地支(戌) 合住 本命時支(卯)",
            NatalBranches(setOf(HOUR), 卯),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 戌),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple1() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            "大運地支(辰) 合住 本命時支(酉)",
            NatalBranches(setOf(HOUR), 酉),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 辰),
          ),
          EwEvent.EwFlow.BranchCombinedDto(
            "流年地支(午) 合住 本命月支、日支(均為 未)",
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 午),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple2() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            ...
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE, FlowScale.YEAR), 午),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.TrilogyToFlowDto(
            ...
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE, FlowScale.YEAR), 辰)
          )
        )
        assertEquals(expected, dtos)
      }
    ...
    inner class 大運流年三合_再拱本命地支 {
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.ToFlowTrilogyDto(
            "大運(辰)、流年(子) 與本命時支(申) 三合 水局",
            NatalBranches(setOf(HOUR), 申),
            setOf(
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 辰),
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 子)
            )
          )
        )
        assertEquals(expected, dtos)
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.ToFlowTrilogyDto(
            ...
            NatalBranches(setOf(YEAR, HOUR), 子),
            setOf(
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE), 辰),
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 申)
            )
          )
        )
        assertEquals(expected, dtos)
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchOppositionDto(
            "流年地支(酉) 正沖 本命時支(卯)",
            NatalBranches(setOf(HOUR), 卯),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 酉)
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchOppositionDto(
            ...
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.LARGE, FlowScale.YEAR), 丑)
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.StemCombinedDto(
            ...
            NatalStems(setOf(HOUR), 己),
            EwEvent.EwFlow.FlowStems(setOf(FlowScale.YEAR, FlowScale.MONTH), 甲),
            FiveElement.土
          )
        )
        assertEquals(expected, dtos)
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            "流月地支(戌) 合住 本命時支(卯)",
            NatalBranches(setOf(HOUR), 卯),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.MONTH), 戌),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple1() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            ...
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.MONTH), 午),
          ),
          EwEvent.EwFlow.BranchCombinedDto(
            "流年地支(辰) 合住 本命時支(酉)",
            NatalBranches(setOf(HOUR), 酉),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 辰),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple2() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchCombinedDto(
            ...
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR, FlowScale.MONTH), 午),
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.TrilogyToFlowDto(
            ...
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR, FlowScale.MONTH), 辰)
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.ToFlowTrilogyDto(
            ...
            setOf(
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 辰),
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.MONTH), 子)
            )
          )
        )
        assertEquals(expected, dtos)
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.ToFlowTrilogyDto(
            ...
            setOf(
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR), 辰),
              EwEvent.EwFlow.FlowBranches(setOf(FlowScale.MONTH), 申)
            )
          )
        )
        assertEquals(expected, dtos)
      ...
      fun single() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchOppositionDto(
            "流月地支(酉) 正沖 本命時支(卯)",
            NatalBranches(setOf(HOUR), 卯),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.MONTH), 酉)
          )
        )
        assertEquals(expected, dtos)
      }
      ...
      fun multiple() {
        ...
        val expected = setOf(
          EwEvent.EwFlow.BranchOppositionDto(
            ...
            NatalBranches(setOf(MONTH, DAY), 未),
            EwEvent.EwFlow.FlowBranches(setOf(FlowScale.YEAR, FlowScale.MONTH), 丑)
          )
        )
        assertEquals(expected, dtos)
      }

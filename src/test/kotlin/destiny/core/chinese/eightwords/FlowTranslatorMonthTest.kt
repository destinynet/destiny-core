  ...
  fun 流年流月同時影響本命天干() {
    ...
    with(affecting) {
      ew.getPatterns(甲辰, 甲戌).translateAffecting().also { list ->
        ...
        assertTrue { list.contains("本命年干、日干(均為 丙) 得到 流年、流月 五行 所生") }
        assertTrue { list.contains("本命月干(乙) 與 流年、流月五行相同") }
        assertTrue { list.contains("本命時干(壬) 生/洩 出流年、流月") }
      }
    }
  }
  ...
  inner class 流年流月合住本命天干 {
    ...
    fun single() {
      ...
      with(stemCombined) {
        ew.getPatterns(甲辰, 甲戌).translateStemCombined().also { list ->
          ...
          assertTrue { list.contains("本命時干(己) 被流年、流月(均為 甲)合住，甲己合化土") }
        }
      }
    }
    ...
    fun multiple() {
      ...
      with(stemCombined) {
        ew.getPatterns(甲辰, 甲戌).translateStemCombined().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("本命日干、時干(均為 己) 被流年、流月(均為 甲)合住，甲己合化土") }
        }
      }
    }
  }
    ...
    fun single() {
      ...
      with(branchCombined) {
        ew.getPatterns(甲辰, 甲戌).translateBranchCombined().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流月地支(戌) 合住 本命時支(卯)") }
        }
      }
    }
    ...
    fun multiple1() {
      ...
      with(branchCombined) {
        ew.getPatterns(甲辰, 庚午).translateBranchCombined().also { list ->
          ...
          assertTrue { list.contains("流月地支(午) 合住 本命月支、日支(均為 未)") }
          assertTrue { list.contains("流年地支(辰) 合住 本命時支(酉)") }
        }
      }
    }
    ...
    fun multiple2() {
      ...
      with(branchCombined) {
        ew.getPatterns(丙午, 丙午).translateBranchCombined().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流年、流月地支(均為 午) 合住 本命月支、日支(均為 未)") }
        }
      }
    }
  }
    ...
    fun single() {
      ...
      with(trilogyToFlow) {
        ew.getPatterns(甲辰, 丙子).translateTrilogyToFlow().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流年(辰)與本命年柱(子)、時柱(申)三合水局") }
        }
      }
    }
    ...
    fun multiple() {
      ...
      with(trilogyToFlow) {
        ew.getPatterns(甲辰, 戊辰).translateTrilogyToFlow().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流年(辰)、流月(辰)與本命年柱(子)、時柱(申)三合水局") }
        }
      }
    }
  }
    ...
    fun single() {
      ...
      with(toFlowTrilogy) {
        ew.getPatterns(甲辰, 丙子).translateToFlowTrilogy().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流年(辰)、流月(子) 與本命時支(申) 三合 水局") }
        }
      }
    }
    ...
    fun multiple() {
      ...
      with(toFlowTrilogy) {
        ew.getPatterns(甲辰, 壬申).translateToFlowTrilogy().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流年(辰)、流月(申) 與本命年支、時支(均為子) 三合 水局") }
        }
      }
    }
  }
  ...
  inner class 流年流月正沖本命地支 {
    ...
    fun single() {
      ...
      with(branchOpposition) {
        ew.getPatterns(甲辰, 癸酉).translateBranchOpposition().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流月地支(酉) 正沖 本命時支(卯)") }
        }
      }
    }
    ...
    fun multiple() {
      ...
      with(branchOpposition) {
        ew.getPatterns(辛丑, 辛丑).translateBranchOpposition().also { list ->
          assertEquals(1, list.size)
          assertTrue { list.contains("流年、流月地支(均為 丑) 正沖 本命月支、日支(均為 未)") }
        }
      }
    }
  }
